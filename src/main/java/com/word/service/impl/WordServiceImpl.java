                ...
                if (parameters != null && parameters.size() > 0) {
                    for (int i = 0; i < parameters.size(); i++) {
                        ...
                        if (param.get("type") == null ){
                            Map<String,String> schema = (Map<String, String>) param.get("schema");
                            String[] sp =  schema.get("$ref").split("/");
                            Map<String ,String > stringMap = new HashMap<>();

                            Map<String,Map<String,String>> properties = (Map<String, Map<String, String>>) definitions.get(sp[sp.length - 1]).get("properties");
                            Iterator<Map.Entry<String, Map<String, String>>> iter = properties.entrySet().iterator();
                            while (iter.hasNext()) {
                                Map.Entry<String, Map<String, String>> info = iter.next();
                                Map<String, String> value1 = info.getValue();

                                Iterator<Map.Entry<String, String>> iterator = value1.entrySet().iterator();
                                while (iterator.hasNext()) {
                                    Map.Entry<String, String> next = iterator.next();
                                    if (next.getKey().equals("type")){
                                        stringMap.put(info.getKey(),next.getValue());
                                    }
                                }
                            }

                            request.setParam(stringMap.toString());
